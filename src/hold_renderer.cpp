...
void HoldRenderer::Draw(const float render_x, const float render_y) const {
  const float wall_ex{wall_size_ /
                      (wall_texture_.GetWidth() * game_const::kResolutionEx)};
  const float box_size{wall_size_ * 5.f};

  if (hold_tetromino_ptr_->IsHold()) {
    const auto hold_tetromino =
        hold_tetromino_ptr_->GetHoldTetromino().GetResetRotation();
    const float block_size = wall_size_ / 1.5f;
    DrawTetrominoCenter(
        hold_tetromino, block_textures_.at(hold_tetromino.GetColor()),
        render_x + box_size / 2.f, render_y + wall_size_ + box_size / 2.f, 1.f,
        block_size);
  }

  // 文字.
  font_view_.Draw(render_x + wall_size_ * 3 / 4.0f,
                  render_y + wall_size_ * 3 / 4.0f, RenderAnchor::TopLeft,
                  "Hold", game_const::kResolutionEx);

  // フレーム.
  if (!hold_tetromino_ptr_->CanHold()) {
    DrawRectAlpha(render_x + wall_size_ / 2.f, render_y + wall_size_ * 1.5f,
                  render_x + wall_size_ * 4.5f, render_y + wall_size_ * 5.5f,
                  0xFF8080, true, 0.3f);
  }

  DrawRect(render_x + wall_size_ / 2.f, render_y + wall_size_ * 1.5f,
           render_x + wall_size_ * 4.5f, render_y + wall_size_ * 5.5f, 0xFFFFFF,
           false);

  for (int x = 0; x < box_size / wall_size_; ++x) {
    wall_texture_.DrawRotated(render_x + wall_size_ * x - wall_size_ / 2.f,
                              render_y - wall_size_ / 2.f,
                              RenderAnchor::TopLeft, wall_ex, 0.0f);

    wall_texture_.DrawRotated(
        render_x + wall_size_ * x - wall_size_ / 2.f,
        render_y - wall_size_ / 2.f + box_size + wall_size_,
        RenderAnchor::TopLeft, wall_ex, 0.0f);
  }

  for (int y = 0; y < box_size / wall_size_ + 2; ++y) {
