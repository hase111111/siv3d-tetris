...
void Field::getField(enumMino(&_field)[TetrisDefine::FIELD_X][TetrisDefine::FIELD_Y]) const
{
...
bool Field::setMino(const enumMino _mino, const int(&_pos)[4][2])
{
	...
	if (_most_low_y < TetrisDefine::FIELD_Y - TetrisDefine::GAMR_OVER_Y)return false;

	//Tspin時の処理
	if (_mino == enumMino::T) {
		int _c = 0;
		if (m_field[_pos[0][0] - 1][_pos[0][1] - 1] != enumMino::none) { _c++; }
		if (m_field[_pos[0][0] + 1][_pos[0][1] - 1] != enumMino::none) { _c++; }
		if (m_field[_pos[0][0] - 1][_pos[0][1] + 1] != enumMino::none) { _c++; }
		if (m_field[_pos[0][0] + 1][_pos[0][1] + 1] != enumMino::none) { _c++; }
...
enumDeleteLine Field::deleteLine()
{
	...
	//値を返す
	if (_delete_line == 1) { return enumDeleteLine::oneline; }
	else if (_delete_line == 2) { return enumDeleteLine::twoline; }
	else if (_delete_line == 3) { return enumDeleteLine::threeline; }
	else if (_delete_line == 4) { return enumDeleteLine::tetris; }
...
{
	if (_x < 0 || _y < 0 || _x >= TetrisDefine::FIELD_X || _y >= TetrisDefine::FIELD_Y) {
		return false;
	}
	else {
		return true;
...
bool Field::anime()
{
	...
	for (int y = 0; y < Define::FIELD_SIZEY; y++) {
		for (int x = 0; x < Define::FIELD_SIZEX; x++) {
			if (m_field[x][y] == enumMino::jammer) { do_anime = true; }
		}
	}

	//処理をせずにfalseを返す
	if (!do_anime) { return false; }

	//5fごとに処理を行う
	m_anime_counter++;
	if (m_anime_counter % 5 != 0) { return true; }

	//ダミーの消去アニメ及び列を下に下げる処理
	for (int y = 0; y < Define::FIELD_SIZEY; y++) {
		for (int x = 0; x < Define::FIELD_SIZEX / 2; x++) {
			if (x < Define::FIELD_SIZEX / 2 - 2 && m_field[Define::FIELD_SIZEX / 2 - 1 - x][Define::FIELD_SIZEY - y - 1] == enumMino::jammer) {
				m_field[Define::FIELD_SIZEX / 2 - 1 - x][Define::FIELD_SIZEY - y - 1] = enumMino::none;
				m_field[Define::FIELD_SIZEX / 2 + x][Define::FIELD_SIZEY - y - 1] = enumMino::none;
				break;
			}
			else {
				if (m_field[Define::FIELD_SIZEX / 2 - 1 - x][Define::FIELD_SIZEY - y - 1] == enumMino::jammer) {
					for (int y2 = y; y2 < Define::FIELD_SIZEY; y2++) {
						for (int x2 = 1; x2 < Define::FIELD_SIZEX - 1; x2++) {
							if (y2 == Define::FIELD_SIZEY - 1) {
								...
								m_field[x2][Define::FIELD_SIZEY - y2 - 1] = m_field[x2][Define::FIELD_SIZEY - y2 - 2];
							}
						}
					}
					y -= 1;
