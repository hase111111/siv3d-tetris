...
TetroMino::TetroMino() : 
	m_fieldx(5),m_fieldy(2),
	m_roll(0),m_counter(TetrisDefine::MINO_FALL_COUNT[1]),m_rock_counter(0), m_rock_operation_counter(0),
	m_rock(false),
...
void TetroMino::setFieldDate(const enumMino(&_date)[TetrisDefine::FIELD_X][TetrisDefine::FIELD_Y])
{
	for (int x = 0; x < TetrisDefine::FIELD_X; x++)
	{
		for (int y = 0; y < TetrisDefine::FIELD_Y; y++)
...
void TetroMino::mf_moveHorizontal()
{
	...
	if(Keyboard::getIns()->getPressingCount(KEY_INPUT_LEFT) >= 1) {
		...
		if (Keyboard::getIns()->getPressingCount(KEY_INPUT_LEFT) == 1 || (Keyboard::getIns()->getPressingCount(KEY_INPUT_LEFT) > TetrisDefine::MINO_HORIZONTAL_INTERVAL &&
			(Keyboard::getIns()->getPressingCount(KEY_INPUT_LEFT) - TetrisDefine::MINO_HORIZONTAL_INTERVAL) % TetrisDefine::MINO_HORIZONTAL_COUNT == 0)) {
			...
			if (mf_canSetMinoInField(m_fieldx - 1, m_fieldy)) {
				...
				if (!mf_canSetMinoInField(m_fieldx + 1, m_fieldy + 1) && !mf_canSetMinoInField(m_fieldx, m_fieldy + 1)) m_rock_operation_counter++;
			}
		}
	}else if (Keyboard::getIns()->getPressingCount(KEY_INPUT_RIGHT) >= 1) {

		//押された瞬間または（インターバル時間以上押されているかつ押した時間ーインターバル時間割るカウンタの余りが０）
		if (Keyboard::getIns()->getPressingCount(KEY_INPUT_RIGHT) == 1 || (Keyboard::getIns()->getPressingCount(KEY_INPUT_RIGHT) > TetrisDefine::MINO_HORIZONTAL_INTERVAL &&
			(Keyboard::getIns()->getPressingCount(KEY_INPUT_RIGHT) - TetrisDefine::MINO_HORIZONTAL_INTERVAL) % TetrisDefine::MINO_HORIZONTAL_COUNT == 0)) {
...
void TetroMino::mf_moveHardDrop()
{
	...
	while (mf_canSetMinoInField(m_fieldx, m_fieldy + 1))
	{
		...
		loop_jammer++;
		if (TetrisDefine::FIELD_Y < loop_jammer) { ERR("無限ループが発生しました"); }
	}
}
...
void TetroMino::mf_moveFreeDrop(int speed)
{
	...
	if (mf_canSetMinoInField(m_fieldx, m_fieldy + 1)) {
		...
		if (speed > 9 || speed < 0) { speed = 9; }	//9を上回るならば9に丸める
